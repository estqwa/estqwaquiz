...
func WebSocketSystemAlertsHandler(hub interface{}) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if shardedHub, ok := hub.(*ShardedHub); ok {
			...
			// Упрощенная проверка для Legacy Hub
			hubMetrics = legacyHub.GetMetrics()

			// Проверяем общую загрузку
			activeConnections := 0
			if conn, ok := hubMetrics["active_connections"].(int64); ok {
			...
			if activeConnections > 5000 {
				...
				// Добавляем алерт о высокой загрузке legacy hub
				alerts = append(alerts, map[string]interface{}{
					"type":     "high_hub_load",
					"severity": "warning",
					"message":  fmt.Sprintf("Высокая загрузка Legacy Hub: %d активных соединений", activeConnections),
					"details": map[string]interface{}{
						"active_connections": activeConnections,
						"recommended_action": "Рекомендуется перейти на ShardedHub для лучшей производительности",
					},
				})
			}
		}
